...
<h3>Yolo Output</h3>
	...
	<p>
		Now for each cell we compute element wise product between objectness score and predicted probabilites of each class. For each of the cells find maximum of probability scores across 3 anchor boxes and all classes. Then box with maximum probability is chosen as the predicted object for the corresponding cell. In practice yolo predicts too many boxes for a single object so we would like to filter that to smaller number of objects. We use Non Maximal Supression to do the same.  
	</p>
<h4>Anchor Boxes</h4>
	<p>
